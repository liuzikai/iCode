...
#pragma data_seg("RemoteHookDll_GETMESSAGE_Data")//设置共享数据段
...
DLLIMPORT int __stdcall SetHook(int TID)//挂钩
{
	...
	hHook=SetWindowsHookEx(WH_GETMESSAGE,iProc,hInsDll,(DWORD)TID);
	if(!hHook)
	{
		char S[25];
		...
		for(int i=1;i<=Msg_list[0];i++)
		{
			if(CWP->message==Msg_list[i])
			{
				...
				for(int j=1;j<=hwnd_list[i][0];j++)
					if(SendMessageA((HWND)hwnd_list[i][j],WM_COPYDATA,0,(LPARAM)&cds))
					{
						CWP->message=0;
						CWP->hwnd=0;
						CWP->wParam=0;
